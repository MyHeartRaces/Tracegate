...
class TracegateApiClient:
    ...
    async def get_device(self, device_id: UUID | str) -> dict:
    ...
    async def get_revision(self, revision_id: UUID | str) -> dict:
        return await self._request("GET", f"/revisions/{revision_id}")
    ...
    async def get_mtproto_access(self, telegram_id: int) -> dict:
        return await self._request("GET", f"/mtproto/access/by-user/{int(telegram_id)}")

    async def list_mtproto_access(self, *, include_revoked: bool = False) -> list[dict]:
        params = None
        if include_revoked:
            params = {"include_revoked": "true"}
        return await self._request("GET", "/mtproto/access", params=params)
    ...
    async def issue_mtproto_access(
        ...
        *,
        ...
        return await self._request("POST", "/mtproto/access/issue", json=payload)

    async def revoke_mtproto_access(self, telegram_id: int) -> dict:
        return await self._request("DELETE", f"/mtproto/access/{int(telegram_id)}")
    ...
    async def accept_bot_welcome(self, telegram_id: int, *, version: str) -> dict:
        return await self._request(
            "POST",
            f"/users/{int(telegram_id)}/bot-welcome-accept",
            json={"version": str(version)},
        )
