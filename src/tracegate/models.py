...
class User(Base):
    ...
    devices_max: Mapped[int] = mapped_column(Integer, default=4, nullable=False)
    ...
    bot_welcome_accepted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    bot_welcome_version: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class MTProtoAccessGrant(Base):
    __tablename__ = "mtproto_access_grant"

    telegram_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("tg_user.telegram_id", ondelete="CASCADE"),
        primary_key=True,
    )
    status: Mapped[RecordStatus] = mapped_column(
        Enum(RecordStatus, name="mtproto_access_status"),
