...
def _telemt_username(raw: object, *, fallback: str) -> str:
...
def build_mtproto_telemt_config(
    *,
    ...
    issued_secret_entries: tuple[MTProtoIssuedSecret, ...] | list[MTProtoIssuedSecret] = (),
    listen_ip: str = "127.0.0.1",
    api_listen: str = "127.0.0.1:9091",
    metrics_listen: str = "127.0.0.1:9090",
    ...
    normalized_public_host = normalize_mtproto_domain(public_host)
    ...
    users: list[tuple[str, str]] = [("tracegate_shared", primary_secret)]
    seen_secrets = {primary_secret}
    seen_users = {"tracegate_shared"}
    for entry in issued_secret_entries:
        try:
        ...
        if len(secret_hex) != 32 or secret_hex in seen_secrets:
            continue
        username = _telemt_username(f"tg_{telegram_id}", fallback=f"tg_{len(users) + 1}")
        while username in seen_users:
            username = _telemt_username(f"{username}_{len(users) + 1}", fallback=f"tg_{len(users) + 1}")
        seen_secrets.add(secret_hex)
        seen_users.add(username)
        users.append((username, secret_hex))
    ...
    lines = [
        ...
        "[general]",
        f"use_middle_proxy = {bool_value(use_middle_proxy)}",
        f"log_level = {_toml_string(str(log_level or 'normal').strip() or 'normal')}",
        "",
        "[general.modes]",
        "classic = true",
        "secure = true",
        ...
        "[server.api]",
        "enabled = true",
        f"listen = {_toml_string(api_listen)}",
        'whitelist = ["127.0.0.1/32", "::1/128"]',
        "",
        "[[server.listeners]]",
        f"ip = {_toml_string(str(listen_ip or '127.0.0.1').strip() or '127.0.0.1')}",
        "",
        "[censorship]",
    ...
    for username, secret_hex in users:
        lines.append(f"{_toml_string(username)} = {_toml_string(secret_hex)}")
    lines.append("")
    return MTProtoTelemtConfig(
        config_text="\n".join(lines),
        accepted_secret_hexes=tuple(secret for _username, secret in users),
...
def build_mtproto_mtg_config(
    *,
    ...
    if not proxy.startswith("socks5://") or proxy == "socks5://":
        raise MTProtoConfigError("MTG requires a non-empty socks5:// egress proxy")
    ...
    client_secret = build_mtproto_client_secret(primary_secret, transport="tls", domain=normalized_tls_domain)
    lines = [
        "# Generated by Tracegate. Do not edit by hand.",
