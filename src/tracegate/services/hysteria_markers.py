...
_LEGACY_RE = re.compile(r"^[Vv]([0-9]+)\s*-\s*([0-9]+)\s*-\s*(.+)$")
_IOS_SAFE_RE = re.compile(r"^[Vv]([0-9]+)_([0-9]+)_([0-9a-fA-F]{32})$")
...
def _normalize_variant(variant: str) -> str:
    ...
    if not raw:
        return "V?"
    if raw[0] in {"v", "V"} and raw[1:].isdigit():
        return f"V{raw[1:]}"
    return raw.upper()
...
def hysteria_ios_safe_username(*, variant: str, tg_id: str | int, connection_id: str) -> str:
    ...
    variant_token = variant_norm.lower() if variant_norm and variant_norm != "V?" else "v"
