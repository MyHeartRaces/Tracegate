...
@router.post("/{telegram_id}/bot-welcome-accept", response_model=UserRead)
async def accept_bot_welcome(
    telegram_id: int,
    payload: UserBotWelcomeAccept,
    session: AsyncSession = Depends(db_session),
) -> UserRead:
    user = await session.get(User, telegram_id)
    if user is None:
    ...
    version = _normalize_profile_part(payload.version)
    if version is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Welcome version is required")

    user.bot_welcome_accepted_at = datetime.now(timezone.utc)
    user.bot_welcome_version = version
