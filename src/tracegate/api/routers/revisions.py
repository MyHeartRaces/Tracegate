...
async def get_revision(revision_id: UUID, session: AsyncSession = Depends(db_session)) -> RevisionRead:
    row = await session.get(ConnectionRevision, revision_id)
    if row is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Revision not found")
    return RevisionRead.model_validate(row, from_attributes=True)
...
async def issue_revision(
    ...
    try:
        ...
        await session.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Revision conflicts with existing state") from exc
