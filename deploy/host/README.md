...
- `/etc/tracegate/deploy.env` selects immutable image digests and paths;
...
2. Run `sudo deploy/host/tracegate-host-install VERSION` from the extracted
   directory. Installation updates `/opt/tracegate/current` atomically but does
   not start the runtime.
...
5. Use `tracegate-host-deploy deploy` for upgrades. It requires an operator
   backup command, pulls pinned images, applies migrations, waits for readiness
