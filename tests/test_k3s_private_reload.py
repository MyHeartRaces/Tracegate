...
def _write_link_crypto_handoff(
    ...
    *,
    ...
    outer_carrier = {
        ...
        "serverName": "www.rbc.ru" if link_class == "entry-transit" else "",
        "publicPort": 443 if link_class == "entry-transit" else 0,
        "publicPath": "/cdn-cgi/tracegate-link" if link_class == "entry-transit" else "",
        "url": "wss://www.rbc.ru:443/cdn-cgi/tracegate-link" if link_class == "entry-transit" else "",
    ...
    _write_env(
        root / "link-crypto" / role_lower / "desired-state.env",
        {
            ...
            "TRACEGATE_LINK_CRYPTO_OUTER_WSS_SERVER_NAME": "www.rbc.ru",
        ...
        "routerClient": {
            ...
            "profileRefs": {
                "shadowsocks2022Client": {
                    "kind": "file",
                    "path": f"/etc/tracegate/private/router/{role_lower}/{link_class}/shadowsocks2022-client.json",
                    "secretMaterial": True,
                }
            },
        },
...
def _write_router_handoff_bundle(root: Path, *, role: str, contract: dict, contract_path: Path) -> None:
    ...
    _write_env(
        root / "router" / role_lower / "client-bundle.env",
        {
            ...
            "TRACEGATE_ROUTER_CLIENT_BUNDLE_COMPONENTS": "sing-box-shadowtls-client" if bundle_routes else "",
...
def test_k3s_private_reload_validates_link_crypto_and_writes_marker(tmp_path: Path) -> None:
    ...
    assert marker["summary"]["carriers"] == ["shadowsocks2022"]
...
def test_k3s_private_reload_link_crypto_validates_router_client_bundle(tmp_path: Path) -> None:
    ...
    assert router["bundle"]["components"]["required"] == ["sing-box-shadowtls-client"]
    ...
    assert router["env"]["clientComponents"] == ["sing-box-shadowtls-client"]
...
def test_k3s_private_reload_fails_on_invalid_handoff(tmp_path: Path) -> None:
    ...
    state["links"][0]["dpiResistance"]["requiredLayers"] = [
        layer for layer in state["links"][0]["dpiResistance"]["requiredLayers"] if layer != "shadowtls-v3"
    ]
    _write_json(state_path, state)
